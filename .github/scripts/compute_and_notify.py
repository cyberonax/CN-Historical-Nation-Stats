...
def send_or_edit(content: str, charts: dict[str, io.BytesIO], nation_csv: io.BytesIO) -> str:
    # Build embeds for each chart
    embeds = [{"image": {"url": f"attachment://{fn}"}} for fn in charts.keys()]
    payload = {"content": content, "embeds": embeds}

    # Attach charts with unique field names
    files = []
    for idx, (fn, buf) in enumerate(charts.items()):
        files.append((f"file{idx}", (fn, buf, "image/png")))
    # Attach the nation-level CSV
...
def main():
    ...
    # Build summary in the desired order
    ...
    csv_buf = io.BytesIO()
    nation_df.to_csv(csv_buf, index=False)
    csv_buf.seek(0)

    # Send or edit the single Discord message
    msg_id = send_or_edit(content, charts, csv_buf)
    save_msg_id(msg_id)
