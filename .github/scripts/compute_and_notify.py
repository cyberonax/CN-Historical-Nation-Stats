...
def delete_old():
    if not Path(MSG_IDS_FILE).exists(): return
    ...
    for m in METRICS:
        if m in agg.columns:
            ...
            if isinstance(v,float):
                lines.append(f"- Current {lbl}: {v:.2f}")
            else:
                lines.append(f"- Current {lbl}: {int(v):,}")
            lines.append(f"- {lbl} Growth/Day: {g:.2f}")
    content="\n".join(lines)
    # generate attachments in order
    attachments=[plot_series(agg[m],LABEL_MAP[m]+" Over Time", f"{m}.png")
                 for m in METRICS if m in agg.columns]
    # post in chunks of 10
