...
# Friendly labels for metrics\LABEL_MAP = {
...
# Exact order of metrics\METRICS = [
    "nation_count",
    "avg_inactivity",
    "Empty_Slots",
    "%_empty",
...
def preprocess(df):
    ...
    for c in ["Technology","Infrastructure","Base Land","Strength",
        ...
        if c in df: df[c]=pd.to_numeric(df[c].astype(str).str.replace(",",""), errors="coerce")
    df["Empty_Slots"]=df[RESOURCE_COLS].apply(lambda r: sum(1 for v in r if pd.isna(v) or str(v).strip()==""), axis=1)//2
    if "Activity" in df: df["avg_inactivity"]=df["Activity"].map(ACTIVITY_MAP)
    return df
...
def post_chunks(content, attachments, chunk_size=10):
    ...
    for i in range(0, len(attachments), chunk_size):
        ...
        pl={"content":content,"embeds":embeds}
        files=[(f"file{j}",(fn,buf,"image/png")) for j,(fn,buf) in enumerate(batch)]
        r=requests.post(WEBHOOK_URL+"?wait=true", data={"payload_json": json.dumps(pl)}, files=files)
        r.raise_for_status()
...
def main():
    ...
    if df.empty:
        print("No data for alliance"); return
    agg=aggregate(df); last=agg.iloc[-1]; ts=last.name.strftime("%Y-%m-%d %H:%M")
    lines=[f"**Aggregated Stats for {TARGET_ALLIANCE} (snapshot {ts})**"]
    for m in METRICS:
        if m in agg.columns:
            v=last[m]; g=growth(agg[m]); lbl=LABEL_MAP[m]
            if pd.isna(v): continue
            if isinstance(v,float): lines.append(f"- Current {lbl}: {v:.2f}")
            else: lines.append(f"- Current {lbl}: {int(v):,}")
            lines.append(f"- {lbl} Growth/Day: {g:.2f}")
    content="\n".join(lines)
    attachments=[plot_series(agg[m], LABEL_MAP[m] + " Over Time", m + ".png") for m in METRICS if m in agg.columns]
    post_chunks(content, attachments)

if __name__=="__main__":
    main()
```

All charts now use `figsize=(7,4)` and are posted in the exact order you listed. Each metric gets its own single‐series graph. You can drop this into your repo directly.```}]}
